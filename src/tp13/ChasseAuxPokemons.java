...
public class ChasseAuxPokemons {
	...
	public static void main(String[] args) {
		...
		try(FileReader lecteur = new FileReader("./InputFile.txt")){
			Scanner s = new Scanner(lecteur);
			while(s.hasNext()) {
				String name = s.next();
				String type = s.next();
		...
		final Joueur moi = new Joueur("S", "Tom", 20, new Pokemon[] {piplup, null, null, null, null});
		...
		try {
			moi.getPokedex().charger("./sauv.txt");
		}
		...
		try {
			moi.getPokedex().sauvegarder("./sauv.txt");
		}
		catch (IOException e) {
